...
export const questions: Question[] = [
  ...
  },
  {
    id: 76,
    type: "multiple",
    tags: ["react", "junior", "portals"],
    body: "Выберите верное(ые) утверждения?",
    availableAnswer: [
      {
        id: 1,
        content:
          "Порталы - позволяют рендерить элемент в DOM-узел, который находится вне DOM-иерархии родительского компонента",
      },
      {
        id: 2,
        content:
          "Чтобы воспользоваться порталом нужно сначало создать портал const portal = React.createPortal(container), а затем вмонтировать нужный компонент portal.render(comp)",
      },
      {
        id: 3,
        content: "Портал может находиться в любом месте DOM-дерева",
      },
      {
        id: 4,
        content:
          "Событие, сгенерированное внутри портала распространяется вверх по дереву DOM-узла где вмонтирован портал, а также вверх по дереву DOM-узла откуда был создан портал",
      },
    ],
    rightAnswer: [1, 3],
  },
